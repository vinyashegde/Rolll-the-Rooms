...
namespace AC
{
	...
	public class ActionListEditorWindow : EditorWindow
	{
		...
		private void CreateEmptyMenu (bool isAsset)
		{
			...
			if (!isAsset)
			{
				menu.AddItem (new GUIContent ("Data/Backup"), false, EmptyCallback, "BackupAll");
				if (windowData.target && windowData.target.HasDataToRestore ())
				{
					menu.AddItem (new GUIContent ("Data/Restore"), false, EmptyCallback, "RestoreAll");
				}
				menu.AddSeparator (string.Empty);
			}
			#endif*/
